...
function build_pio_sketch(){ # build_pio_sketch <board> <path-to-ino>
	...
	python -m platformio ci -l '.' --board "$board" "$sketch_dir" --project-option="board_build.partitions = huge_app.csv" --project-option="build_flags=$defines"
}
...
function build_pio_sketches() # build_pio_sketches <board> <examples-path> <chunk> <total-chunks>
{
    ...
    for sketch in $sketches; do
        ...
        local sketchDefines
        while read line; do
            sketchDefines="$sketchDefines -D$line"
        ...
        build_pio_sketch "$board" "$sketch" "$sketchDefines"
        local result=$?
        if [ $result -ne 0 ]; then
            return $result
        fi
