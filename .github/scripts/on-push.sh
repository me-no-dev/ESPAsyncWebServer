...
if [ "$BUILD_PIO" -eq 0 ]; then
	...
	if [[ "$TARGET_PLATFORM" == "esp32" ]]; then
		...
		git clone https://github.com/OpenShock/AsyncTCP "$ARDUINO_USR_PATH/libraries/AsyncTCP" > /dev/null 2>&1
		FQBN="espressif:esp32:esp32:PSRAM=enabled,PartitionScheme=huge_app"
		source ./.github/scripts/install-arduino-core-esp32.sh
		echo "BUILDING ESP32 EXAMPLES"
	else
		echo "Installing ESPAsyncTCP ..."
		git clone https://github.com/OpenShock/ESPAsyncTCP "$ARDUINO_USR_PATH/libraries/ESPAsyncTCP" > /dev/null 2>&1
	...
	if [[ "$TARGET_PLATFORM" == "esp32" ]]; then
		BOARD="esp32dev"
		echo "Installing AsyncTCP ..."
		python -m platformio lib -g install https://github.com/OpenShock/AsyncTCP.git > /dev/null 2>&1
		echo "BUILDING ESP32 EXAMPLES"
	else
		BOARD="esp12e"
		echo "Installing ESPAsyncTCP ..."
		python -m platformio lib -g install https://github.com/OpenShock/ESPAsyncTCP.git > /dev/null 2>&1
		echo "BUILDING ESP8266 EXAMPLES"
	fi
