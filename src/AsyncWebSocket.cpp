...
int max_ws_clients = DEFAULT_MAX_WS_CLIENTS;
int max_ws_queued_messages = WS_MAX_QUEUED_MESSAGES;
...
bool AsyncWebSocketClient::queueIsFull(){
  if((_messageQueue.length() >= max_ws_queued_messages) || (_status != WS_CONNECTED) ) return true;
  return false;
}
...
void AsyncWebSocketClient::_queueMessage(AsyncWebSocketMessage *dataMessage){
  ...
  if(_messageQueue.length() >= max_ws_queued_messages){
