...
AsyncStaticWebHandler::AsyncStaticWebHandler(const char *uri, FS &fs,
                                             const char *path,
...
bool AsyncStaticWebHandler::_fileExists(AsyncWebServerRequest *request,
                                        const String &path) {
...
void AsyncStaticWebHandler::handleRequest(AsyncWebServerRequest *request) {
  ...
  if (request->hasHeader("Range")) {
    ...
    if (rh.startsWith("bytes=")) {
      ...
      char *ptr;
      uint32_t start = strtoul(tmp.c_str(), &ptr, 10);
      tmp = rh.substring(dash + 1, rh.length());
      uint32_t end = strtoul(tmp.c_str(), &ptr, 10);

      DEBUGF("Range start=%u length=%u\n", start, end - start);
      AsyncWebServerResponse *response =
          new AsyncFileResponse(request->_tempFile, filename, String(), false,
                                _callback, start, end);
      request->send(response);
    }
  } else if (request->_tempFile == true) {
    ...
    if (_last_modified.length() &&
      ...
      AsyncWebServerResponse *response = new AsyncFileResponse(
